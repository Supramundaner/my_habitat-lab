...
def select_room_with_llm(topdown_path: str, room_annotation_path: str, 
    ...
    while retry_count < max_retries and selected_room is None:
        try:
            # Set up client with OpenRouter (no need to instantiate OpenAI)
                        ...
                        "content": [
                            {
                                "type": "image_url", 
                                "image_url": {
                                    "url": f"data:image/png;base64,{encode_image(room_annotation_path)}"
                                },
                            },
                            {
                                "type": "text",
...
if __name__ == "__main__":
    ...
    # Validate and set API key if using LLM
    if manual_room is None:
        if not config['llm_config'].get('api_key'):
            raise ValueError("API key is missing in the configuration file. Please provide a valid API key under 'llm_config'.")
        openai.api_key = config['llm_config']['api_key']
        openai.api_base = config['llm_config']['base_url']
