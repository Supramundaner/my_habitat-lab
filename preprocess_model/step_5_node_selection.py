...
def call_llm_with_images(original_image_path, nodes_image_path, prompt_text, model, extra_headers=None):
...
def select_node_with_llm(original_room_image: np.ndarray, 
    ...
    model = llm_config.get('model', 'qwen/qwen-2.5-72b-instruct')
    max_tokens = llm_config.get('max_tokens', 1000)
    max_retries = llm_config.get('max_retries', 3)
    site_url = llm_config.get('site_url', '')
    site_name = llm_config.get('site_name', '')
                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            },
                            {
                                "type": "image_url", 
                                "image_url": {
                                    "url": f"data:image/png;base64,{encode_image(temp_original_image_path)}"
                                },
                            },
                            {
                                "type": "image_url", 
                                "image_url": {
                                    "url": f"data:image/png;base64,{encode_image(temp_nodes_image_path)}"
                                },
                            },
                        ],
                    },
...
if __name__ == "__main__":
    ...
    if not config['llm_config'].get('api_key'):
        raise ValueError("API key is missing in the configuration file. Please provide a valid API key under 'llm_config'.")

    # Set the API key explicitly
    openai.api_key = config['llm_config']['api_key']
