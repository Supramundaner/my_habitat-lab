"""
Step 4: LLM room selection using visual analysis or text description.
Enhanced version that uses room annotation and either goal image or text description for better selection.
"""
...
def call_llm_with_text(client, room_annotation_path, text_description, prompt_text, model):
    """Call LLM API with room annotation image and text description"""
    # Encode room annotation image
    room_annotation_base64 = encode_image(room_annotation_path)
...
def select_room_with_llm(topdown_path: str, room_annotation_path: str, goal_path: str,
                        config: Dict[str, Any], output_dir: str, orchestrator_data: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    Use LLM to select target room from room annotation and goal image or text description.
    
    Args:
        topdown_path: Path to topdown view image
        room_annotation_path: Path to room annotation image
        goal_path: Path to goal image or text description file
    ...
    if use_text_nav:
        print(f"📁 Loading files for LLM analysis (TextNav mode):")
        print(f"  - Topdown: {topdown_path}")
        print(f"  - Room annotation: {room_annotation_path}")
        print(f"  - Goal description: {goal_path}")
        
        # Check if files exist
        for path, name in [(topdown_path, "topdown"), (room_annotation_path, "room annotation")]:
            if not os.path.exists(path):
                raise FileNotFoundError(f"{name.capitalize()} image not found: {path}")
        
        if not os.path.exists(goal_path):
            raise FileNotFoundError(f"Goal description file not found: {goal_path}")
        
        # Load text description
        with open(goal_path, 'r', encoding='utf-8') as f:
            text_description = f.read().strip()
        
        print(f"📝 Text description loaded: {len(text_description)} characters")
        
    else:
        print(f"📁 Loading images for LLM analysis (Image mode):")
        print(f"  - Topdown: {topdown_path}")
        print(f"  - Room annotation: {room_annotation_path}")
        print(f"  - Goal image: {goal_path}")
        
        # Check if images exist
        for path, name in [(topdown_path, "topdown"), (room_annotation_path, "room annotation"), (goal_path, "goal image")]:
            if not os.path.exists(path):
                raise FileNotFoundError(f"{name.capitalize()} image not found: {path}")
    ...
    # Get goal object (needed for both modes)
    goal_object = config['scene_config']['goal_object']
    
    # Load prompt template
    if use_text_nav:
        # Use text-specific prompt
        text_prompt_path = os.path.join(os.path.dirname(config['prompts']['choose_room_prompt']), 
                                      'choose_room_prompt_text.txt')
    ...
    if use_text_nav:
        text_desc = config['scene_config'].get('text_description', text_description)
        enhanced_prompt = prompt_template.format(text_description=text_desc)
        print(f"📝 Using text description for room selection")
    else:
...
Format: Final Answer: [room_number]"""
    ...
    while retry_count < max_retries and selected_room is None:
        ...
        try:
            # Call LLM
            if use_text_nav:
                response = call_llm_with_text(
                    client, room_annotation_path, text_description, enhanced_prompt, model
                )
            else:
                response = call_llm_with_images(
                    client, room_annotation_path, goal_path, enhanced_prompt, model
                )
