...
Enhanced for image-instance navigation with goal image guidance or text description.
"""
...
def call_llm_with_text(client, original_image_path, nodes_image_path, text_description, prompt_text, model):
    """Call LLM API with two images and text description"""
    # Encode images
    original_image_base64 = encode_image(original_image_path)
    nodes_image_base64 = encode_image(nodes_image_path)
...
def select_node_with_llm(original_room_image: np.ndarray, 
    ...
    """Use LLM to select optimal navigation node using images and goal image or text description."""
        ...
        try:
            ...
            if use_text_nav:
                response = call_llm_with_text(
                    client, temp_original_path, temp_nodes_path, text_description,
                    enhanced_prompt, model
                )
            else:
                response = call_llm_with_images(
                    client, temp_original_path, temp_nodes_path, goal_path,
                    enhanced_prompt, model
                )
