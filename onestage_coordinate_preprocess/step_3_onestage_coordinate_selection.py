...
def select_coordinate_with_llm(image: np.ndarray, 
    ...
    print(f"  - Normalize size: {normalize_size}x{normalize_size}")
    ...
    normalized_image_path = os.path.join(output_dir, "normalized_topdown.png")
    ...
    while retry_count < max_retries and selected_coordinate is None:
        try:
            ...
            # 调用LLM API
            response = call_llm_with_image(client, normalized_image_path, prompt_template, model)
            ...
            print(f"📝 Extracted coordinate: {extracted_coord}")
            ...
            if extracted_coord is not None:
                ...
                if 0 <= x <= normalize_size and 0 <= y <= normalize_size:
                    # 如果启用了验证，检查是否在可导航区域
                    if validate_coords:
                        if validate_coordinate((x, y), normalized_image):
                            selected_coordinate = (x, y)
                            print(f"✓ Valid coordinate selected: {selected_coordinate}")
                            break
                        else:
                            print(f"⚠️ Coordinate {extracted_coord} is not in navigable area")
                    else:
                        selected_coordinate = (x, y)
    ...
    if selected_coordinate is None:
        ...
        print(f"⚠️ All LLM attempts failed. Using fallback coordinate: {selected_coordinate}")
