...
class MultiAgentSimulator:
    ...
    def __init__(self, config: Dict):
        ...
        self.collision_check_steps = self.movement_config.get("collision_check_steps", 1) # 新增：碰撞检测步数
    ...
    def execute_actions_sequence(self, actions: Dict[str, List[Any]]) -> bool:
        ...
        try:
            ...
            for step in range(max_actions):
                ...
                # 预测碰撞 - 使用新的动态碰撞检测
                collision_detected, collision_reason = self._predict_dynamic_collisions(current_step_actions)
            ...
            if len(agent_actions) > 0 and agent_actions[0].action == "move_to":
                ...
                if target and len(target) >= 2:
                    ...
                    if position is not None:
                        ...
                        # 使用新的姿态更新方法
                        self._update_agent_pose(agent_id, position, self.agent_states[agent_id].rotation)
    ...
    def _sample_path_for_action(self, action: ActionCommand, agent_state: AgentState) -> list:
        ...
        if action.action == "move_to" and action.target:
            ...
            if target_pos is not None:
                for i in range(1, num_steps + 1):
                    t = i / num_steps
                    interpolated_pos = agent_state.position + t * (target_pos - agent_state.position)
                    path.append(interpolated_pos)
            ...
            for i in range(1, num_steps + 1):
                dist_step = action.distance * (i / num_steps)
                movement = np.array([forward_dir.x, 0, forward_dir.z]) * dist_step
                path.append(agent_state.position + movement)
    ...
    def _update_agent_pose(self, agent_id: str, position: np.ndarray, rotation: Union[np.ndarray, mn.Quaternion]):
        ...
        if agent_id in self.agent_robots:
            ...
            try:
                ...
                # 应用变换
                robot_obj.transformation = translation_matrix @ rotation_matrix
            except Exception as e:
                logging.debug(f"Failed to update robot pose for {agent_id}: {e}")
