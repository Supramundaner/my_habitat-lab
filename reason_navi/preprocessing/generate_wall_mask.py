...
def generate_wall_mask(topdown_path: str, output_dir: str) -> Dict[str, Any]:
    ...
    if not cv2.imwrite(wall_mask_path, wall_mask):
        raise OSError(f"Failed to write wall mask: {wall_mask_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "test":
        ...
        print(
            "Usage: python3 -m reason_navi.preprocessing.generate_wall_mask "
            "<topdown_path> <output_dir>"
        )
        print(
            "   or: python3 -m reason_navi.preprocessing.generate_wall_mask test"
        )
        sys.exit(1)
