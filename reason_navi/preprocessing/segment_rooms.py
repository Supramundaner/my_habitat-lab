...
def segment_rooms(topdown_path: str, wall_mask_path: str, metadata_path: str,
    ...
    if not cv2.imwrite(segmentation_path, final_segmented_image):
        raise OSError(f"Failed to write room segmentation: {segmentation_path}")
    ...
    if not cv2.imwrite(annotation_path, annotated_image):
        raise OSError(f"Failed to write room annotation: {annotation_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        print(
            "Usage: python3 -m reason_navi.preprocessing.segment_rooms "
    ...
    result = segment_rooms(topdown_path, wall_mask_path, metadata_path, config, output_dir)
