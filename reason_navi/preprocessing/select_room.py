...
def call_llm_with_image(
    client, room_annotation_path, prompt_text, model, max_tokens=1000
):
    """调用LLM API进行推理"""
    # 编码room annotation图片
...
def get_available_rooms(output_dir: str) -> List[int]:
    """Read room IDs produced by Step 2 in the current workflow run."""
    step2_result_path = os.path.join(output_dir, "step2_results.json")
    if not os.path.isfile(step2_result_path):
        raise FileNotFoundError(
            f"Step 2 room results not found: {step2_result_path}"
        )
    try:
        with open(step2_result_path, 'r', encoding='utf-8') as f:
            step2_data = json.load(f)
        room_bboxes = step2_data.get("results", {}).get(
            "room_bounding_boxes", {}
        )
        if not isinstance(room_bboxes, dict):
            raise ValueError("room_bounding_boxes must be a JSON object")
        room_ids = sorted(int(room_id) for room_id in room_bboxes)
    except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
        raise RuntimeError(
            f"Failed to load current Step 2 room results: {step2_result_path}"
        ) from exc
    if not room_ids:
        raise RuntimeError("Step 2 produced no available rooms")
    print(f"✓ Found available rooms from step2_results.json: {room_ids}")
    return room_ids

def select_room(topdown_path: str, room_annotation_path: str,
    ...
    while retry_count < max_retries and selected_room is None:
        try:
            if Ark is None:
                raise RuntimeError(
                    "byteplussdkarkruntime is required for BytePlus Ark; install "
                    "it with `python3 -m pip install byteplussdkarkruntime`"
                )
    ...
    if selected_room is None:
        if fallback_on_failure and available_rooms:
            ...
            raise RuntimeError(
                "LLM room selection failed after "
    ...
    with open(log_path, 'w', encoding='utf-8') as f:
        json.dump(llm_log, f, indent=2, ensure_ascii=False, allow_nan=False)
    print(f"✓ LLM interaction log saved to: {log_path}")
    
    # 保存LLM的完整content和reasoning content到预处理位置
    preprocessing_details_dir = os.path.join(output_dir, "preprocess")
    os.makedirs(preprocessing_details_dir, exist_ok=True)
    ...
    # Keep the historical artifact directory name for output compatibility.
    llm_content_path = os.path.join(
        preprocessing_details_dir, "llm_room_selection_content.json"
    )
    with open(llm_content_path, 'w', encoding='utf-8') as f:
        json.dump(
            llm_content_data, f, indent=2, ensure_ascii=False, allow_nan=False
        )
    print(f"✓ LLM content saved to preprocessing directory: {llm_content_path}")
    
    # 同时保存一个简化的reasoning文本文件，便于阅读
    reasoning_text_path = os.path.join(
        preprocessing_details_dir, "llm_reasoning.txt"
    )
...
def select_room_manually(room_annotation_path: str, output_dir: str, room_number: int = 1) -> Dict[str, Any]:
    ...
    with open(log_path, 'w', encoding='utf-8') as f:
        json.dump(manual_log, f, indent=2, ensure_ascii=False, allow_nan=False)
    ...
    # Keep the historical artifact directory name for output compatibility.
    manual_content_path = os.path.join(
        preprocessing_details_dir, "llm_room_selection_content.json"
    )
    with open(manual_content_path, 'w', encoding='utf-8') as f:
        json.dump(
            manual_content_data, f, indent=2, ensure_ascii=False, allow_nan=False
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python3 -m reason_navi.preprocessing.select_room "
            "<topdown_path> <room_annotation_path> <config_path> "
            "[manual_room_number]"
        )
    ...
    if manual_room is not None:
        ...
        result = select_room(topdown_path, room_annotation_path, config, output_dir)
