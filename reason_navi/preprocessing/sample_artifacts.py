...
def find_scene_directory(
    scene_name: str, val_dir: Optional[os.PathLike] = None
) -> str:
    """
    ...
    # Resolve the dataset from this checkout unless the caller overrides it.
    resolved_val_dir = Path(val_dir) if val_dir else (
        REPO_ROOT / "data/versioned_data/hm3d-0.2/hm3d/val"
    )
    pattern = str(resolved_val_dir / f"*{scene_name}")
...
def create_config_for_object(
    base_config_path: str,
    obj: Dict[str, Any],
    output_config_path: str,
    scene_root: Optional[os.PathLike] = None,
) -> str:
    """
    为特定object创建配置文件。
    ...
    Returns:
        创建的配置文件路径
    """
    # The loader strips any legacy plaintext API key from the serializable dict.
    config = load_preprocessing_config(base_config_path)
    ...
    try:
        val_dir = (
            Path(scene_root).expanduser().resolve(strict=False)
            if scene_root is not None
            else Path(
                os.environ.get(
                    "HABITAT_SCENE_ROOT",
                    str(REPO_ROOT / "data/versioned_data/hm3d-0.2/hm3d/val"),
                )
            ).expanduser().resolve(strict=False)
        )
        actual_dir_name = find_scene_directory(scene_name, val_dir)
        scene_path = val_dir / actual_dir_name / f"{scene_name}.basis.glb"
        ...
        scene_path = (
            val_dir
            / scene_name
            / f"{scene_name}.basis.glb"
        )

    config['scene_config']['scene_path'] = str(scene_path)
    config['scene_config']['target_coordinate'] = obj['position']
    config['scene_config']['goal_object'] = obj['object_category']
    config.setdefault('output', {})['output_dir'] = str(
        Path(output_config_path).expanduser().resolve().parent
    )
    
    # 保存配置文件（运行时密钥不会被复制）
    write_preprocessing_config(config, output_config_path)
...
def run_room_segmentation_for_object(config_path: str, output_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if __package__:
            ...
            from pipeline import PreprocessingPipeline
        orchestrator = PreprocessingPipeline(config_path)
        # 更新输出目录
...
def batch_generate_samples(
    ...
    for i, obj in enumerate(sampled_objects):
        ...
        try:
            create_config_for_object(
                base_config_path, obj, config_path, scene_root=scene_root
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate deterministic two-stage preprocessing samples"
    )
    parser.add_argument("json_path")
    parser.add_argument("base_config_path")
    parser.add_argument("output_dir")
    parser.add_argument("num_samples", nargs="?", type=int, default=10)
    parser.add_argument(
        "--scene-root",
        default=os.environ.get("HABITAT_SCENE_ROOT"),
        help=(
            "HM3D split directory containing scene folders; defaults to "
            "$HABITAT_SCENE_ROOT or the checkout data directory"
        ),
    )
    ...
    # 运行批量生成
    results = batch_generate_samples(
        json_path,
        base_config_path,
        output_dir,
        num_samples,
