    ...
    def sample_region(self, walkable_mask: np.ndarray, region_mask: np.ndarray, 
        ...
        if not initial_point:
            ...
            while attempts < 1000:
                ...
                idx = self.rng.randint(0, len(region_coords[0]) - 1)
        ...
        while self.active_list:
            ...
            active_index = self.rng.randint(0, len(self.active_list) - 1)
            ...
            for _ in range(self.max_attempts):
                ...
                distance = self.rng.uniform(self.radius, 2 * self.radius)
...
def sample_multiple_regions(
    ...
    all_points = []
    point_id_offset = 0
    sampling_rng = rng if rng is not None else random.Random()
...
def generate_navigation_graph(topdown_path: str, wall_mask_path: str, metadata_path: str,
    ...
    random_seed = graph_config.get('seed')
    ...
    if not cv2.imwrite(graph_with_topdown_path, graph_image):
        raise OSError(
            f"Failed to write graph visualization: {graph_with_topdown_path}"
        )
    ...
    with open(nodes_json_path, 'w', encoding='utf-8') as f:
        json.dump(
            nodes_json_data, f, indent=2, ensure_ascii=False, allow_nan=False
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        print(
            "Usage: python3 -m "
            "reason_navi.preprocessing.generate_navigation_graph "
            "<topdown_path> <wall_mask_path> <metadata_path> <config_path>"
        )
        sys.exit(1)
