...
def render_topdown_view(glb_path, target_floor, custom_ortho_scale=None, target_coverage=0.9, draw_coordinates=False, resolution=[2048, 2048]):
    ...
    temp_sim = robust_load_ortho_sim(
        glb_path, ortho_scale=1.0, resolution=resolution
    )
    try:
        ...
        if custom_ortho_scale is not None:
            ...
            print(
                "Scene visual dimensions: "
                f"{scene_info['width']:.2f}m x {scene_info['depth']:.2f}m"
            )
            print(
                f"Maximum visual dimension: {scene_info['max_dimension']:.2f}m"
            )
            print(
                "Scene visual center: "
                f"X={scene_info['center'][0]:.2f}, "
                f"Z={scene_info['center'][1]:.2f}"
            )
    ...
    if isinstance(target_floor, int):
        if 0 <= target_floor < len(floor_extents):
            ...
            selected_floor_index = target_floor
    ...
    if final_image is None:
        raise RuntimeError("Failed to render the target floor")
    if unprojected_coords is None:
        raise RuntimeError("Top-down render did not provide world coordinates")

    meta_data = calculate_metadata(unprojected_coords)
    meta_data["selected_floor"] = {
        "index": int(selected_floor_index),
...
if __name__ == '__main__':
    # --- 配置 ---
    try:
        # Override HABITAT_SCENE_PATH when using a dataset outside this checkout.
        repo_root = Path(__file__).resolve().parent.parent
        default_scene_path = (
            repo_root
            / "data/versioned_data/hm3d-0.2/hm3d/val"
            / "00808-y9hTuugGdiq/y9hTuugGdiq.basis.glb"
        )
        hm3d_scene_path = os.environ.get(
            "HABITAT_SCENE_PATH", str(default_scene_path)
        )
