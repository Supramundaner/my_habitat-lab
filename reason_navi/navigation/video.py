...
class VideoComposer:
    ...
    def __init__(self, simulator: HabitatSimulator, config: Dict[str, Any], output_path: str):
        ...
        try:
            ...
            if base_map is None:
                raise RuntimeError(
                    "Simulator did not provide the required top-down base map"
                )
            ...
            print(
                "视频合成器初始化完成: "
                f"{self.video_width}x{self.video_height} @ {self.fps}fps"
            )
        except Exception:
    ...
    def add_frame(self, robot_state: Optional[Dict[str, Any]] = None, 
        ...
        if self.occupancy_map:
            ...
            occupancy_map_img = self.occupancy_map.get_map_image(
                robot_state,
        ...
        # 4. 调整图像尺寸 - 保持纵横比
        right_panel_height = self.video_height // 2  # 改为二等分
        ...
        if self.video_writer is None:
            raise RuntimeError("Cannot add a frame after the video writer is closed")
    ...
    def set_occupancy_map(self, occupancy_map, config: Dict[str, Any]):
        """设置地图构建器并同步坐标系"""
        ...
        if topdown_metadata:
            ...
            occupancy_map.set_global_reference(
