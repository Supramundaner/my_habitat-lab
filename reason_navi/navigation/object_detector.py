...
from ..vlm.runtime_config import (
    GROUNDING_DINO_PORT,
    MOBILE_SAM_PORT,
    YOLOV7_PORT,
)
...
class ObjectDetector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        supported_detectors = {"grounding_dino", "yolov7"}
        if self.detector_type not in supported_detectors:
            supported = ", ".join(sorted(supported_detectors))
            raise ValueError(
                f"Unsupported object detector {self.detector_type!r}; expected one of: {supported}"
            )

        # Client construction is deliberately fail-fast. When detection is enabled,
        # an import/configuration failure must not silently turn ObjectNav into a
        # detector-free policy.
        mobile_sam_port = self.config.get('mobile_sam_port') or MOBILE_SAM_PORT
        self.mobile_sam = MobileSAMClient(port=mobile_sam_port)

        if self.detector_type == 'yolov7':
            detector_port = self.config.get('yolov7_port') or YOLOV7_PORT
            self.detector_client = YOLOv7Client(port=detector_port)
        else:
            detector_port = self.config.get('grounding_dino_port') or GROUNDING_DINO_PORT
            self.detector_client = GroundingDINOClient(port=detector_port)

        print("Object detector initialized successfully")
        print(f"Detector type: {self.detector_type}")
        print(f"Detector port: {detector_port}")
    ...
    def detect_object(
        ...
        try:
            ...
            raise RuntimeError(
                f"{self.detector_type} detection failed for target {target_object!r}. "
                "Verify that the detector and Mobile SAM loopback services are running."
            ) from exc
    ...
    def project_to_3d(
        ...
        try:
            ...
            raise RuntimeError("Failed to project the detection mask into 3D") from exc
    ...
    def calculate_target_position(
        ...
        try:
            ...
            raise RuntimeError("Failed to calculate the detected target position") from exc
