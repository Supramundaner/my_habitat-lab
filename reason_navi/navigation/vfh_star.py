...
class VFHStar:
    ...
    def get_best_direction(self, robot_pos: np.ndarray, robot_theta: float, 
                          ...
                          prev_direction: float = None, occupancy_map = None) -> Optional[float]:
        """
        获取最佳移动方向
        ...
        Args:
            ...
            obstacles: 障碍物列表 [(x, y, radius), ...]（可选，如果提供occupancy_map则忽略）
            prev_direction: 前一个选择的方向
            occupancy_map: MapBuilder实例（可选，如果提供则直接从地图计算）
        ...
        # 获取候选方向
        primary_candidates = self._get_candidate_directions(robot_pos, obstacles, occupancy_map)
    ...
    def _get_polar_histogram(self, pos: np.ndarray, obstacles: List[Tuple[float, float, float]] = None, occupancy_map = None) -> np.ndarray:
        """
        计算极坐标直方图
        
        Args:
            pos: 机器人位置 
            obstacles: 障碍物列表（可选，如果提供occupancy_map则忽略）
            occupancy_map: MapBuilder实例（可选，如果提供则直接从地图计算）
            
        Returns:
            极坐标直方图数组
        """
        # 如果提供了occupancy_map，使用高效的直接地图计算方法
        if occupancy_map is not None:
            return self._get_polar_histogram_from_map(pos, occupancy_map)
    ...
    def _get_polar_histogram_from_map(self, pos: np.ndarray, occupancy_map) -> np.ndarray:
        """
        从地图直接计算极坐标直方图（内部方法）
        
        Args:
            pos: 机器人位置 [x, z]
            occupancy_map: MapBuilder实例
        ...
        # 将机器人位置转换为地图坐标
        agent_map_coords = occupancy_map._world_to_map_coords(np.array([[pos[0], 0, pos[1]]]))
        agent_map_x, agent_map_y = int(agent_map_coords[0, 0]), int(agent_map_coords[0, 1])
        
        # 计算传感器范围内的地图像素半径
        sensor_radius_pixels = int(self.sensor_range / occupancy_map.map_resolution)
        robot_radius_pixels = max(1, int(self.robot_radius / occupancy_map.map_resolution))
        ...
        # 转换为世界坐标的相对位置
        rel_world_x = x_grid * occupancy_map.map_resolution
        rel_world_z = y_grid * occupancy_map.map_resolution  # 注意：y对应z轴
        ...
        # 应用掩码
        valid_mask = ((abs_x >= 0) & (abs_x < occupancy_map.map_shape[1]) &
                      (abs_y >= 0) & (abs_y < occupancy_map.map_shape[0]))
        distance_mask = (distances >= self.robot_radius) & (distances <= self.sensor_range)
        combined_mask = valid_mask & distance_mask
        
        if np.any(combined_mask):
            # 获取有效像素的占用状态
            valid_occupancy = occupancy_map.grid_map[abs_y[combined_mask], abs_x[combined_mask]]
    ...
    def _get_candidate_directions(self, pos: np.ndarray, obstacles: List[Tuple[float, float, float]] = None, occupancy_map = None) -> List[float]:
        """
        获取候选方向
        ...
        histogram = self._get_polar_histogram(pos, obstacles, occupancy_map)
