"""
NavigationController - 动作处理和动画逻辑类 (Controller)
协调整个模拟过程，处理目标导航和动画
"""
...
from .camera import pinhole_intrinsics
...
class NavigationController:
    ...
    def __init__(self, simulator: HabitatSimulator, composer: VideoComposer, config: Dict[str, Any], occupancy_map=None):
        """
        初始化动作处理器
        ...
        # 初始化导航配置
        self.nav_config = NavigationSettings.from_config(config)
        ...
        print(f"中间waypoint距离: {self.nav_config.intermediate_distance} m")
        print(f"最终目标距离: {self.nav_config.final_stop_threshold} m")
    ...
    def _should_switch_to_object_detection(self, current_pos_2d: np.ndarray, current_path: List[np.ndarray], target_pos_2d: np.ndarray) -> bool:
        ...
        return path_distance < self.nav_config.detection_switch_distance
    
    def _get_camera_params(self, image_shape) -> Dict[str, float]:
        """Derive intrinsics from the actual observation and configured HFOV."""
        if image_shape is None or len(image_shape) < 2:
            raise ValueError("camera observation must expose height and width")
        height, width = int(image_shape[0]), int(image_shape[1])
        hfov = self.config.get('OCCUPANCY_MAP', {}).get('HFOV', 90.0)
        return pinhole_intrinsics(width, height, hfov)
    ...
    def _camera_to_world_coords(self, camera_coords: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # 基于occupancy_map.py中的_transform_points_to_world函数实现
            ...
            raise RuntimeError(
                "Failed to transform detected camera coordinates into world coordinates"
            ) from exc
    ...
    def _select_vfh_target(self, current_pos_2d: np.ndarray, adjusted_target_pos: np.ndarray, 
        ...
        if path_distance_to_target < self.nav_config.detection_switch_distance:
    ...
    def navigate(self, action_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        执行目标导航序列
    ...
    def _execute_hybrid_phase(self, current_pos_2d: np.ndarray, current_rot: np.ndarray, 
        ...
        # 检查目标点是否在障碍物内
        if self.occupancy_map.grid_map is not None:
            # 将世界坐标转换为地图坐标
            adjusted_target_map_coords = self.occupancy_map._world_to_map_coords(np.array([[adjusted_target_pos[0], 0, adjusted_target_pos[1]]]))
            adjusted_target_pixel = (int(adjusted_target_map_coords[0, 0]), int(adjusted_target_map_coords[0, 1]))
            
            target_value = self.occupancy_map.grid_map[adjusted_target_pixel[1], adjusted_target_pixel[0]]
            if target_value == 0:  # 障碍物
                print(f"[WARNING] adjusted_target_pos在障碍物内，寻找最近的可行走点")
                resolution = self.occupancy_map.map_resolution
                nearest_walkable_pixel = self._find_nearest_walkable_point(adjusted_target_pixel, self.occupancy_map.grid_map, resolution)
        ...
        if result.get('failed', False):
            return {
                ...
                'message': result.get('message', 'VFH navigation failed'),
            }
    ...
    def _execute_object_detection_phase(self, current_pos_2d: np.ndarray, current_rot: np.ndarray, 
        ...
        if detected_target_pos is None:
            ...
            # 相机参数
            camera_params = self._get_camera_params(depth_image.shape)
        ...
        if self.occupancy_map.grid_map is not None:
            # 将世界坐标转换为地图坐标
            target_map_coords = self.occupancy_map._world_to_map_coords(np.array([[target_pos_2d[0], 0, target_pos_2d[1]]]))
            target_pixel = (int(target_map_coords[0, 0]), int(target_map_coords[0, 1]))
            
            target_value = self.occupancy_map.grid_map[target_pixel[1], target_pixel[0]]
            if target_value == 0:  # 障碍物
                print(f"[WARNING] 目标点在障碍物内，寻找最近的可行走点")
                resolution = self.occupancy_map.map_resolution
                nearest_walkable_pixel = self._find_nearest_walkable_point(target_pixel, self.occupancy_map.grid_map, resolution)
                if nearest_walkable_pixel is not None:
                    # 将像素坐标转换为世界坐标
                    nearest_walkable_world = self._pixel_to_world_coord(nearest_walkable_pixel)
        ...
        if current_path_result is None:
            return {
                'success': False,
                'failed': True,
                'reason': 'no_a_star_path_to_detection_target',
                'message': 'No reachable A* path to the detection target',
            }
        current_path = current_path_result['path']
            ...
            if detected_target_pos is None:
                ...
                if rotation_result['success']:
                    ...
                    return {
                        'success': False,
                        'failed': True,
                        'reason': rotation_result.get(
                            'reason', 'target_not_found_in_rotation_search'
                        ),
                        'message': rotation_result.get(
                            'message', '到达预测位置后未检测到目标'
                        ),
                    }
            else:
        ...
        return {
            ...
            'target_pos_2d': updated_original_target_pos,
        }
    ...
    def _execute_vfh_navigation(self, current_pos_2d: np.ndarray, current_rot: np.ndarray,
        ...
        # 从占用地图提取局部障碍物
        obstacles = self.occupancy_map.get_obstacles_from_map(current_pos_2d, vfh_star.sensor_range)
        
        # 获取机器人朝向角度
        robot_theta = self.occupancy_map.get_robot_theta_from_quaternion(current_rot)
    ...
    def _plan_a_star_path(self, start_pos: np.ndarray, goal_pos: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 获取占用地图
            if self.occupancy_map.grid_map is None:
                print("[ERROR] 占用地图未初始化")
                return None
            
            map = self.occupancy_map.grid_map.copy()
            ...
            # 将世界坐标转换为地图坐标
            start_map_coords = self.occupancy_map._world_to_map_coords(np.array([[start_pos[0], 0, start_pos[1]]]))
            goal_map_coords = self.occupancy_map._world_to_map_coords(np.array([[goal_pos[0], 0, goal_pos[1]]]))
            ...
            was_adjusted = False
            resolution = self.occupancy_map.map_resolution*4
            if goal_value == 0:
                print(f"[WARNING] 目标点在障碍物内，寻找最近的可行走点")
    ...
    def _add_robot_padding(self) -> np.ndarray:
        ...
        # 机器人半径（像素）
        robot_radius_pixels = max(1, int(0.14 / self.occupancy_map.map_resolution))  # 0.14m机器人半径
    ...
    def _a_star_pathfinding(self, start: tuple, goal: tuple, wall_mask: np.ndarray) -> List[tuple]:
        ...
        visited = set()
        iterations = 0
        max_iterations = self.nav_config.a_star_max_iterations
    ...
    def _pixel_to_world_coord(self, pixel: tuple) -> np.ndarray:
        ...
        """
        # 使用occupancy_map的逆变换
        world_x = (pixel[0] + 0.5) * self.occupancy_map.map_resolution + self.occupancy_map.topdown_map_bounds['top_left'][0]
        world_z = (pixel[1] + 0.5) * self.occupancy_map.map_resolution + self.occupancy_map.topdown_map_bounds['top_left'][1]
    ...
    def _is_safe_walkable_point(self, pixel: tuple, map: np.ndarray, safety_radius_m: float = 0.1) -> bool:
        ...
        # 将安全半径转换为像素单位
        safety_radius_pixels = int(np.ceil(safety_radius_m / self.occupancy_map.map_resolution))
    ...
    def _execute_rotation_search(self, target_name: str, vfh_star: VFHStar, 
        ...
        for step in range(self.rotation_search_steps):
            ...
            if detected_target_pos_camera is not None:
                ...
                if self.occupancy_map.grid_map is not None:
                    target_map_coords = self.occupancy_map._world_to_map_coords(np.array([[detected_target_2d[0], 0, detected_target_2d[1]]]))
                    target_pixel = (int(target_map_coords[0, 0]), int(target_map_coords[0, 1]))
                ...
                navigation_result = self._navigate_to_detected_target(
                    detected_target_2d,
                    vfh_star,
                    max_iterations=self.nav_config.detected_target_max_iterations,
                )
