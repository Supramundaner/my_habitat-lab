...
from .runtime_config import MOBILE_SAM_PORT, required_file
...
try:
    ...
    _MOBILE_SAM_IMPORT_ERROR = exc
else:
    _MOBILE_SAM_IMPORT_ERROR = None
...
class MobileSAM:
    def __init__(
        ...
        checkpoint_file = required_file(
            sam_checkpoint,
            env_var="MOBILE_SAM_CHECKPOINT",
            cli_flag="--checkpoint",
            description="Mobile SAM checkpoint",
        )
        if _MOBILE_SAM_IMPORT_ERROR is not None:
            raise RuntimeError(
                "Mobile SAM server dependency is unavailable. Install mobile_sam "
                "in the service environment."
            ) from _MOBILE_SAM_IMPORT_ERROR
        if device is None:
            device = torch.device("cuda") if torch.cuda.is_available() else "cpu"
        self.device = device

        mobile_sam = sam_model_registry[model_type](checkpoint=str(checkpoint_file))
...
class MobileSAMClient:
    def __init__(self, port: int = MOBILE_SAM_PORT):
        self.url = f"http://127.0.0.1:{port}/mobile_sam"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=MOBILE_SAM_PORT)
    parser.add_argument(
        "--checkpoint",
        help="Checkpoint path (or set MOBILE_SAM_CHECKPOINT)",
    )
    ...
    mobile_sam = MobileSAMServer(sam_checkpoint=args.checkpoint)
    print("Model loaded!")
