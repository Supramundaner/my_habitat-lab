"""
多楼层 Topdown 渲染模块
基于 current_topdown.py 实现，支持针对特定楼层的单独渲染
"""
...
class MultiFloorTopdownRenderer:
    ...
    def analyze_scene(self, custom_ortho_scale: Optional[float] = None, target_coverage: float = 0.9):
        ...
        if custom_ortho_scale is not None:
            ...
            self.optimal_ortho_scale = calculate_ortho_scale(self.scene_info['max_dimension'], target_coverage)
        
        self.floor_extents = get_floor_navigable_extents(temp_sim)
        self.floor_extents = sorted(self.floor_extents, key=lambda x: x['mean'])
