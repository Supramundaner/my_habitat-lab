    ...
    def _draw_coordinate_system(self, image: Image.Image) -> Image.Image:
        ...
        while x_current <= world_max_x:
            ...
            if 0 <= px < width:
                ...
                draw.text((px - text_width//2, -20), label, fill=border_color, font=font_small)
                draw.text((px - text_width//2, height + 5), label, fill=border_color, font=font_small)
        ...
        while z_current <= world_max_z:
            ...
            if 0 <= py < height:
                ...
                draw.text((-35, py - 6), label, fill=border_color, font=font_small)
                draw.text((width + 5, py - 6), label, fill=border_color, font=font_small)
        ...
        # 添加坐标轴标签（参考图片样式）
        # X轴标签（顶部中央）
        draw.text((width//2 - 20, -35), "X (meters)", fill=border_color, font=font_large)
        ...
        temp_draw.text((0, 0), z_label, fill=border_color, font=font_large)
        ...
        draw.text((10, height - 25), scale_info, fill=border_color, font=font_small)
        ...
        draw.text((compass_x+18, compass_y-15), "+X", fill=border_color, font=font_small)
        draw.text((compass_x-8, compass_y+18), "+Z", fill=border_color, font=font_small)
...
class HabitatNavigatorApp(QMainWindow):
    ...
    def process_view_command(self, command: str):
        ...
        try:
            ...
            # 应用视角变化
            self.simulator.rotate_sensor(pitch_change, yaw_change)
            self.update_fpv_display()
            
            self.status_label.setText(f"视角已调整: {direction} {angle}度")
            
        except ValueError:
            self.status_label.setText("角度必须是数字")
    ...
    def animate_movement(self):
        ...
        if self.current_interpolation_step == 0:
            ...
            if np.linalg.norm(direction) > 0:
                ...
                # 计算旋转四元数（面向移动方向）
                # 在Habitat中，-Z方向是前方
                target_forward = np.array([direction[0], 0, direction[2]])
                
                # 计算Y轴旋转角度
                angle = math.atan2(target_forward[0], -target_forward[2])
