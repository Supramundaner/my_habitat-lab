...
# Assuming run_eval.py is in a structure like: project/onestage_eval/run_eval.py
...
try:
    ...
    print(f"Fatal: Could not import OneStageWorkflowOrchestrator from main_workflow_onestage.py: {e}")
    print("Please ensure main_workflow_onestage.py is in the correct path (onestage_preprocess/).")
    sys.exit(1)
...
class EpisodeEvaluator:
    ...
    def _create_preprocess_config(self, episode_data: Dict[str, Any]) -> Path:
        """Create preprocessing configuration file for one-stage method."""
        try:
            preprocess_config = {
                ...
                "wall_mask": {
                    "morph_closing_width_meters": self.config['preprocess'].get('room_segmentation', {}).get('morph_closing_width_meters', 0.01)
                },
                "graph_generation": self.config['preprocess']['graph_generation'],
                "llm_config": self.config['preprocess']['llm_config'],
                "prompts": {
                    "choose_node_prompt": str(self.preprocess_dir / "prompts" / "choose_onestage_node.txt")
                },
                "output": {
            ...
            print(f"✓ Created one-stage preprocessing config: {preprocess_config_path}")
    ...
    def _run_preprocessing(self, preprocess_config_path: Path) -> bool:
        ...
        try:
            print("\n" + "="*60)
            print("RUNNING ONE-STAGE PREPROCESSING PIPELINE (IN-PROCESS)")
            print("="*60)
            os.chdir(self.preprocess_dir)
            orchestrator = OneStageWorkflowOrchestrator(str(preprocess_config_path))
            success = orchestrator.run_workflow()
            if success:
                print("✓ One-stage preprocessing completed successfully")
                return True
            else:
                error_msg = "One-stage preprocessing workflow failed."
                self.results["errors"].append(error_msg)
                print(f"✗ {error_msg}")
                return False
        except Exception as e:
            error_msg = f"Failed to run one-stage preprocessing: {e}"
            self.results["errors"].append(error_msg)
            print(f"✗ {error_msg}")
            traceback.print_exc()
            return False
