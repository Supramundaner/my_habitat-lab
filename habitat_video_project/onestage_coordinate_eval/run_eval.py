...
# Assuming run_eval.py is in a structure like: project/onestage_coordinate_eval/run_eval.py
# and main.py is in project/video/
# and main_workflow_onestage.py is in project/onestage_coordinate_preprocess/
project_root = current_dir.parent.parent  # Go up two levels to reach habitat-lab
onestage_preprocess_root = project_root / "onestage_coordinate_preprocess"
video_root = current_dir.parent 

sys.path.insert(0, str(onestage_preprocess_root))
sys.path.insert(0, str(video_root))

# Imports from main_workflow_onestage.py
try:
    from main_workflow_onestage import OneStageCoordinateWorkflowOrchestrator
except ImportError as e:
    print(f"Fatal: Could not import OneStageCoordinateWorkflowOrchestrator from main_workflow_onestage.py: {e}")
    print("Please ensure main_workflow_onestage.py is in the correct path (onestage_coordinate_preprocess/).")
    sys.exit(1)
...
class EpisodeEvaluator:
    ...
    def __init__(self, config_path: str):
        ...
        self.preprocess_dir = onestage_preprocess_root
    ...
    def _create_preprocess_config(self, episode_data: Dict[str, Any]) -> Path:
        """Create preprocessing configuration file for one-stage coordinate method."""
        try:
            preprocess_config = {
                ...
                "scene_config": {
                    ...
                    "target_floor": self.config['preprocess']['scene_config'].get('target_floor', 0),
                ...
                "prompts": {
                    "choose_coordinate_prompt": str(self.preprocess_dir / "prompts" / "choose_coordinate.txt")
                },
                "coordinate_selection": {
                    "image_normalize_size": 1000,
                    "coordinate_validation": self.config['preprocess'].get('coordinate_selection', {}).get('coordinate_validation', False)
                },
                "output": {
            ...
            print(f"✓ Created one-stage coordinate preprocessing config: {preprocess_config_path}")
    ...
    def _run_preprocessing(self, preprocess_config_path: Path) -> bool:
        """Run one-stage preprocessing pipeline by direct import."""
        original_cwd = os.getcwd()
        try:
            print("\n" + "="*60)
            print("RUNNING ONE-STAGE COORDINATE PREPROCESSING PIPELINE (IN-PROCESS)")
            print("="*60)
            os.chdir(self.preprocess_dir)
            orchestrator = OneStageCoordinateWorkflowOrchestrator(str(preprocess_config_path))
            success = orchestrator.run_workflow()
            if success:
                print("✓ One-stage coordinate preprocessing completed successfully")
                return True
            else:
                error_msg = "One-stage coordinate preprocessing workflow failed."
                self.results["errors"].append(error_msg)
                print(f"✗ {error_msg}")
                return False
        except Exception as e:
            error_msg = f"Failed to run one-stage coordinate preprocessing: {e}"
            self.results["errors"].append(error_msg)
            print(f"✗ {error_msg}")
            traceback.print_exc()
            return False
    ...
    def _create_video_generation_assets(self) -> Dict[str, Path]:
        ...
        try:
            ...
            # Note: coordinate-based method doesn't generate wall_mask, so we don't require it
            return {
                "config_path": video_config_path,
                "actions_path": preprocess_action_path,
                "wall_mask_path": None  # Not available in coordinate-based method
            }
        except Exception as e:
    ...
    def _run_video_generation(self, assets: Dict[str, Path]) -> bool:
        ...
        try:
            ...
            # Add wall_mask only if available (not in coordinate-based method)
            if assets.get('wall_mask_path'):
                args['wall_mask'] = str(assets['wall_mask_path'])
    ...
    def run_evaluation(self) -> bool:
        ...
        try:
            print("\n" + "🚀" + "="*58 + "🚀")
            print("🎯 STARTING ONE-STAGE EPISODE EVALUATION (REFACTORED) 🎯")
            print("🚀" + "="*58 + "🚀")
            ...
            print("\n2. Running one-stage preprocessing...")
            ...
            if not self.results["preprocessing_success"]:
                print("✗ One-stage preprocessing failed, stopping evaluation.")
                return False
            ...
            print("\n" + "🎉" + "="*58 + "🎉")
            print("🎯 ONE-STAGE EVALUATION COMPLETED SUCCESSFULLY! 🎯")
            print("🎉" + "="*58 + "🎉")
