...
def select_navigation_node(graph_path: str, topdown_path: str, room_bbox: Dict[str, int], 
    ...
    Args:
        ...
        topdown_path: Path to topdown view image (may contain markers from previous iterations)
    ...
    # 保存 room_with_graph with iteration-specific naming
    room_with_graph_filename = f"room_with_graph_iter_{iteration + 1}.png" if iteration > 0 else "room_with_graph.png"
    room_with_graph_path = os.path.join(output_dir, room_with_graph_filename)
    ...
    # Save final result with iteration-specific naming
    node_with_topdown_filename = f"node_with_topdown_iter_{iteration + 1}.png" if iteration > 0 else "node_with_topdown.png"
    node_with_topdown_path = os.path.join(output_dir, node_with_topdown_filename)
    ...
    # Use iteration-specific log file name
